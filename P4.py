...
def modulador16QAM(bits, fc, mpp):
    ...
    :return: Un vector con la señal modulada
    :return: Un valor con la potencia promedio [W]
    :return: La onda portadora s(t)
    :return: La onda cuadrada moduladora (información)
    '''
    # 1. Parámetros de la 'señal' de información (bits)
    N = len(bits) # Cantidad de bits
    newbits = bits.reshape(-1, 4) #creo un vector 
    

    # 2. Construyendo un periodo de la señal portadora s(t)
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)  # mpp: muestras por período
    portadorasin = np.sin(2*np.pi*fc*t_periodo)
    portadoracos = np.cos(2*np.pi*fc*t_periodo)
    ...
    i=0
    for b1, b2, b3, b4 in newbits:
        if b1 == 0 and b2 == 0:
            senal_Tx[i*mpp : (i+1)*mpp] = portadoracos * -3
            moduladora[i*mpp : (i+1)*mpp] = -3
        if b1 == 0 and b2 == 1:
            senal_Tx[i*mpp : (i+1)*mpp] = portadoracos * -1
            moduladora[i*mpp : (i+1)*mpp] = -1
        if b1 == 1 and b2 == 1:
            senal_Tx[i*mpp : (i+1)*mpp] = portadoracos * 1
            moduladora[i*mpp : (i+1)*mpp] = 1
        if b1 == 1 and b2 == 0:
            ...
            moduladora[i*mpp : (i+1)*mpp] = 3
        if b3 == 0 and b4 == 0:
            senal_Tx[(i+2)*mpp : (i+3)*mpp] = portadorasin * 3
            moduladora[(i+2)*mpp : (i+3)*mpp] = 3
        if b3 == 0 and b4 == 1:
            senal_Tx[(i+2)*mpp : (i+3)*mpp] = portadorasin * 1
    ...
    portadora = portadorasin + portadoracos
...
def demodulador16QAM(senal_Rx, portadora, mpp):
    '''Un método que simula un bloque demodulador
    ...
    # Matriz acomoda el vector señal de entrada (pasar de vector a matriz) 
    matriz = np.zeros((int(N/4),4))
    
    # Creo los vector para la señal demodulada
    senal_demoduladasin = np.zeros(senal_Rx.shape)
    senal_demoduladacos = np.zeros(senal_Rx.shape)
    ...
    # Demodulación
    # j es la variable que recorre el vector de entrada o la senal_Rx
    # i es la variable que recorre las filas de la matriz
    j=0
    for i in range(int(N/4)):
        # Producto interno de dos funciones
        productocos = senal_Rx[j*mpp : (j+1)*mpp] * portadora
        productosin = senal_Rx[(j+2)*mpp : (j+3)*mpp] * portadora
        Epc = np.sum(productocos)/10
        Eps = np.sum(productosin)/10
        senal_demoduladacos[j*mpp : (j+1)*mpp] = Epc
        senal_demoduladasin[(j+2)*mpp : (j+3)*mpp] = Eps
        j=j+4 #ya se usaron obtuvieron los primeros 4 bits del vector por lo que se aumenta en 4 el...
              #contador para obtener los siguentes 4 bits en la siguiente iteración
        
        # Criterio de decisión por detección de energía
        '''
        Para tomar en cuenta el ruido no se iguala a los valores de amplitud sino que 
        se elige un rango dentro del cual se decida si se trata de una amplitud u otra.
        1 = Ep > 0 y Ep < 2
        3 = Ep > 2
        -1 = Ep < 0 y Ep > -2
        -3 = Ep < -2
    ...
    bits = matriz.reshape(-1) #Se reforma la matriz a un vector que sera retornado para imprimir la imagen
    senal_demodulada = senal_demoduladasin + senal_demoduladacos
    
    return bits.astype(int), senal_demodulada


# In[11]:


# Funcion principal que llama a las funciones
import numpy as np
import matplotlib.pyplot as plt
import time
...
senal_Tx, Pm, portadora, moduladora = modulador16QAM(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador16QAM(senal_Rx, portadora, mpp)
...
print("Gráficas de la señal moduladora, señal transmitida, señal recibida y señal modulada recibida respectivamente")
...
X_t = np.empty((8, len(t_muestra)))	   # 16 funciones del tiempo x(t) 


#Obtengo e imprimo el promedio estadistico
Prom_estadistico = np.average(senal_Tx)
print("El promedio estadistico de la senal es:{:0.4f}".format(Prom_estadistico))



#Algoritmo para obtener el promedio temporal
...
T = Ns * Tc
t_simulacion = np.linspace(0, Ns*Tc, Ns*mpp) 
Prom_temporal = (1 / T) * np.trapz(senal_Tx, t_simulacion)
print("El promedio temporal de la senal es:{:0.4f}".format(Prom_temporal))
diferencia = Prom_temporal-Prom_estadistico
if diferencia < 0.0001 and diferencia > -0.0001:
    print("Como el promedio estadístico es igual al promedio temporal quiere decir que el proceso es ergódico")
    print("Además como el proceso es ergódico implica que también es estacionario.")
else:
    print("Como el promedio estadístico es diferente al promedio temporal quiere decir que el proceso no es ergódico")


    
# Creación de la matriz con los valores de cada función posibles   
for i in A:
...
# Promedio de las 16 realizaciones en cada instante (cada punto en t)
P = [np.mean(X_t[:,i]) for i in range(len(t_muestra))]
plt.plot(t_muestra, P, lw=6,color='g',label='Promedio Realizaciones')

# Graficar el resultado teórico del valor esperado
E = np.mean(senal_Tx)*t_muestra  # Valor esperado de la señal 
plt.plot(t_muestra, E, '-.', lw=3,color='b',label='Valor teórico')
...
# Gráfica
plt.title("Densidad espectral de potencia de la señal modulada Tx")
